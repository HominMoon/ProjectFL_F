	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<UALSCharacterMovementComponent>(CharacterMovementComponentName))
{
	...
	//AddOwnedComponent(ClimbingComp);
}
...
void AALSBaseCharacter::RagdollStart()
{
	...
	//custom for ragdoll recovery by time
	//RagdollRecovery();
}

void AALSBaseCharacter::RagdollRecovery()
{
	//FTimerHandle Timer;
	//GetWorld()->GetTimerManager().SetTimer(Timer, this, AALSBaseCharacter::RagdollEnd, LandedVelocity / 100, false);
}
...
void AALSBaseCharacter::EventOnLanded()
{
	...
	UE_LOG(LogTemp, Warning, TEXT("%f"), VelZ);


	if (bRagdollOnLand && VelZ > RagdollOnLandVelocity)
	{
		//use this ALSCharacter override
...
void AALSBaseCharacter::ForwardMovementAction_Implementation(float Value)
{
	...
	//ClimbSystem
	/*else if (MovementState == EALSMovementState::Climb)
	{
...
void AALSBaseCharacter::RightMovementAction_Implementation(float Value)
{
	...
	{
		if (ClimbingComp->MovingSideLedge) return;
		if (bCanMoveRight && (MovingLeftRight > 0.f))
		{
			//1. 키가 눌리면 해당 방향의 포인트가 넥스트 포인트로 생성됨
			//2. 레퍼런스 포인트가 넥스트 포인트로 바뀜
			//3. 넥스트 포인트로 이동하는 도중에는 애니메이션 재생하고 이동 중에는 다른 입력값 받을 수 없음
			ClimbingComp->MoveNextPoint();

		}
		if (bCanMoveLeft && (MovingLeftRight < 0.f))
		{
			ClimbingComp->MoveNextPoint();
		}
	...
	if (bValue)
	{
		...
		if (MovementAction == EALSMovementAction::None)
		{
			if (MovementState == EALSMovementState::Grounded)
			{
				if (Stance == EALSStance::Standing)
				{
					UE_LOG(LogTemp, Warning, TEXT("check standing"));
					Jump();
				}
				else if (Stance == EALSStance::Crouching)
				{
