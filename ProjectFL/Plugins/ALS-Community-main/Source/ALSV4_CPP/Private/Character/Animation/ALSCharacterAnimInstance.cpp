...
void UALSCharacterAnimInstance::NativeInitializeAnimation()
{
	...
	Player = Cast<AALSCharacter>(GetOwningActor());
...
void UALSCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	//custom info
	CanMoveLeft = Character->bCanMoveLeft;
	CanMoveRight = Character->bCanMoveRight;

	//interface value
	Player->ClimbingComp->ClimbValues_Implementation(ClimbState, MovingLeft, MovingRight, FullHang);
